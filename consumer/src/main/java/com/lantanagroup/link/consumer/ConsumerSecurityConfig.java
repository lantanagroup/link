...
public class ConsumerSecurityConfig extends WebSecurityConfigurerAdapter {
  ...
  protected void configure(HttpSecurity http) throws Exception {
    ...
    authFilter.setAuthenticationManager(new LinkAuthManager(this.config.getIssuer(), this.config.getAlgorithm(), this.config.getAuthJwksUrl(), this.config.getTokenVerificationClass(),null));
    http
            .csrf().disable()
            .cors()
            .and()
